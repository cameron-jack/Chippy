...
class JackknifeTests(TestCase):
    ...
    def test_proper_initialise(self):
        ...
        self.assertEqual(test_knife._run, False)
    ...
    def test_jackknife_stats(self):
        ...
        pmcc_stat = stat_maker(pmcc, data, 1)
        test_knife = JackknifeStats(data.shape[1], pmcc_stat)
        self.assertAlmostEquals(test_knife.JackknifedStatistic, 1.2905845)
        self.assertAlmostEquals(test_knife.StandardStatError, 0.2884490)
        self.assertEqual(test_knife._run, True)

        # Vector
        mean_stat = stat_maker(mean, data, 1)
        test_knife = JackknifeStats(data.shape[1], mean_stat)
        expected_jk_stat = data.mean(axis=1)
        got_jk_stat = test_knife.JackknifedStatistic
        expected_standard_err = [30.69509346, 1.87179671]
        got_standard_err = test_knife.StandardStatError
    ...
    def test_tables(self):
        ...
        #print test_knife.SummaryStatistics
